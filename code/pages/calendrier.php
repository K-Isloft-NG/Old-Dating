<?php
// calendrier.php - Calendrier des après-midis de rencontre
...
// Données
$evenements  = getEvenementsRencontreDuMois($mois, $annee);
$selectedEv  = $selectedId ? getEvenementParId($selectedId) : ($evenements[0] ?? null);
...
<head>
  ...
  <title>Old Dating — Calendrier</title>
...
<div class="app-wrapper">
  ...
  <main class="main-content" id="contenu-principal">
    ...
    <div class="page-header">
      <div class="breadcrumb"><a href="dashboard.php">Accueil</a> › Calendrier des rencontres</div>
      <div class="page-title">📅 Calendrier des rencontres</div>
    </div>
      ...
      <div>
        ...
        <div class="cal-grid">
          ...
          <div class="cal-body">
            ...
            ?>
              <a href="?mois=<?= $mois ?>&annee=<?= $annee ?>&ev=<?= $firstEvId ?? '' ?>"
        ...
        <div style="margin-top:24px">
          <div class="section-title">Tous les événements de <?= $moisFr[$mois] ?></div>
          <?php if (empty($evenements)): ?>
            <div class="card" style="text-align:center;color:var(--muted);padding:24px">
      ...
      <div>
        <?php if ($selectedEv): ?>
          ...
          <div class="card" style="border-left:4px solid var(--teal)">
            <div class="flex-gap-8 mb-16" style="flex-wrap:wrap;display:flex">
              <?php if ($selectedEv['categorie']): ?>
                <span class="badge badge-teal"><?= htmlspecialchars($selectedEv['categorie']) ?></span>
              <?php endif; ?>
              <?php if ($complet): ?>
                <span class="badge badge-gray">Complet</span>
            ...
            <?php if ($complet): ?>
              <button class="btn btn-ghost btn-full" disabled>Événement complet 🔒</button>
            <?php elseif ($dejaInscrit): ?>
              <div class="alert alert-success">✅ Vous êtes inscrit(e) à cet événement !</div>
            <?php else: ?>
        ...
        <?php else: ?>
          <div class="card" style="text-align:center;color:var(--muted);padding:40px 24px">
            <span style="font-size:2.5rem;display:block;margin-bottom:12px">📅</span>
            Sélectionnez un jour dans le calendrier pour voir le détail d'un événement.
          </div>
        <?php endif; ?>
      </div>
